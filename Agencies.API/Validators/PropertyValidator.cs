...
namespace Agencies.API.Validators
{
    ...
    public class UpdatePropertyRequestValidator : AbstractValidator<UpdatePropertyRequest>
    {
        ...
        {
            // Все поля optional для обновления
            ...
            RuleFor(x => x.Price)
                ...
                .LessThanOrEqualTo(1000000000).When(x => x.Price > 0)
                .WithMessage("Цена не должна превышать 1 000 000 000");

            // Area = 0 означает "не обновлять"
            RuleFor(x => x.Area)
                .GreaterThan(0).When(x => x.Area > 0)
                .WithMessage("Площадь должна быть больше 0")
                .LessThanOrEqualTo(10000).When(x => x.Area > 0)
                .WithMessage("Площадь не должна превышать 10 000 м²");

            RuleFor(x => x.Type)
                .Must(BeValidPropertyType).When(x => !string.IsNullOrEmpty(x.Type))
                .WithMessage("Недопустимый тип недвижимости");

            // Rooms = -1 означает "не обновлять" (по умолчанию), 0 - допустимое значение
            RuleFor(x => x.Rooms)
                .InclusiveBetween(0, 100).When(x => x.Rooms >= 0)
                .WithMessage("Количество комнат должно быть от 0 до 100");
